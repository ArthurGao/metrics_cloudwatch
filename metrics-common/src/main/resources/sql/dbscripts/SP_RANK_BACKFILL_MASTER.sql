...
DELIMITER ;;
CREATE PROCEDURE `SP_RANK_BACKFILL_MASTER`(
IN date_from DATE,
IN date_to DATE
)
BEGIN
	...
	SELECT `datestr` FROM `date_lookup_prod` WHERE datestr >= date_from AND datestr <= date_to ORDER BY datestr DESC;
	...
	OPEN cursor_ranges;
        ...
        range_loop: LOOP
            ...
            CALL `metrics`.`SP_RANK_COMPUTE_DATA`(rank_date);
